...
$today_payment="SELECT *FROM `matatu_payments` WHERE timestamp LIKE '%$date%' AND matatu_id='$matid' ORDER BY timestamp DESC";
$today_paymentq=mysqli_query($con,$today_payment);

if(mysqli_num_rows($today_paymentq)>0){
    
    echo '<h4>Matatu payments sorted according to date : '.$date.'</h4>';
    ...
    while($row=mysqli_fetch_array($today_paymentq)){
        ...
        while($row=mysqli_fetch_array($fetch_mat_detailsq)){
          $plateno=$row['plate_no'];
            ...
            $sum="SELECT sum(amount) FROM `matatu_payments` WHERE timestamp LIKE '%$date%' AND matatu_id='$matid'";
   ...
   echo 'NO PAYMENTS FOR THIS DATE '.$date; 
    exit();
}
